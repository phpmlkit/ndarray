...
/// Compute sum along axis using proper strided view.
fn sum_axis_view(
    ...
    unsafe {
        ...
        if let Some(view) = extract_view_f64(wrapper, offset, shape, strides) {
            return view.sum_axis(Axis(axis));
        }
        // Float32 - convert to f64 for sum
        if let Some(view) = extract_view_f32(wrapper, offset, shape, strides) {
            return view.mapv(|x| x as f64).sum_axis(Axis(axis));
        }
        // Int64 - convert to f64 for sum
...
pub unsafe extern "C" fn ndarray_sum_axis(
    ...
    crate::ffi_guard!({
        ...
        // Compute sum along axis using proper strided view
        let result_arr = sum_axis_view(wrapper, offset, shape_slice, strides_slice, axis_usize);

        // Handle keepdims
        let _out_shape = compute_axis_output_shape(shape_slice, axis_usize, keepdims);
        ...
        // Create wrapper with appropriate dtype
