...
/// Returns a 0-dimensional array handle containing the product value.
#[no_mangle]
pub unsafe extern "C" fn ndarray_product(
    ...
    crate::ffi_guard!({
        ...
        let product_val = view.iter().fold(1.0, |acc, &x| acc * x);

        // Create scalar result wrapper with same dtype as input
        let result_wrapper = create_scalar_wrapper(product_val, wrapper.dtype);
