//! Cumulative product over flattened array (axis=None equivalent).
//!
//! Flattens the array in C-order and returns 1D cumulative product.
    ...
    crate::ffi_guard!({
        ...
        let result_wrapper = match wrapper.dtype {
            DType::Float64 => {
                ...
                let flat = view.flatten().into_owned();
                let result = flat.cumprod(Axis(0)).into_dyn();
                NDArrayWrapper {
            ...
            DType::Float32 => {
                ...
                let result = view.flatten().into_owned().cumprod(Axis(0)).into_dyn();
                NDArrayWrapper {
            ...
            DType::Bool => {
                crate::error::set_last_error("cumprod() not supported for Bool type".to_string());
                return ERR_GENERIC;
            }
        };
