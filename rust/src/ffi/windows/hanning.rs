...
fn hanning_symmetric(m: usize) -> Vec<f64> {
    if m == 0 {
        return vec![];
    }
    if m == 1 {
    ...
    for n in 0..m {
        ...
        w.push(0.5 - 0.5 * a.cos());
    }
    w
}

fn hanning_window(m: usize, periodic: bool) -> Vec<f64> {
    if periodic && m > 1 {
        let mut w = hanning_symmetric(m + 1);
        w.pop();
        w
    } else {
        hanning_symmetric(m)
...
#[no_mangle]
pub unsafe extern "C" fn ndarray_hanning(
    m: usize,
    periodic: bool,
    ...
    crate::ffi_guard!({
        let data = hanning_window(m, periodic);
        let arr: ArrayD<f64> = match ArrayD::from_shape_vec(IxDyn(&[m]), data) {
            Ok(a) => a,
            Err(e) => {
                set_last_error(format!("hanning(): shape error: {}", e));
                return ERR_GENERIC;
            }
        };
