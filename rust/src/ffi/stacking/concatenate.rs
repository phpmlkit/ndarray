...
/// shapes: pointer to num_arrays * ndim values (row-major: array i at [i*ndim .. (i+1)*ndim])
...
pub unsafe extern "C" fn ndarray_concatenate(
    ...
    crate::ffi_guard!({
        ...
        let first_handle = match handles_slice.get(0) {
            Some(&h) => h,
            None => {
                set_last_error("concatenate requires at least one array".to_string());
                return ERR_GENERIC;
            }
        };
        ...
        for &h in handles_slice.iter().skip(1) {
            ...
            if w.dtype != dtype {
                set_last_error("concatenate requires arrays with same dtype".to_string());
                return ERR_DTYPE;
            }
        }

        let shape_0 = slice::from_raw_parts(shapes, ndim);
        let axis_usize = match resolve_axis(shape_0, axis) {
            Ok(a) => a,
            Err(e) => {
                set_last_error(e);
                return ERR_SHAPE;
