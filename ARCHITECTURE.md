...
│   ├── libndarray_php.so         # Linux
│   ├── libndarray_php.dylib      # macOS
│   └── ndarray_php.dll           # Windows
│
...
**Auto-generated C Header** (`lib/ndarray_php.h`):
...
class FFIInterface
{
    ...
    private static function findLibrary(): string
    {
        ...
        if (PHP_OS_FAMILY === 'Windows') {
            return $baseDir . 'ndarray_php.dll';
        } elseif (PHP_OS_FAMILY === 'Darwin') {
            return $baseDir . 'libndarray_php.dylib';
        } else {
            return $baseDir . 'libndarray_php.so';
        }
    }
...
```bash
# Clone repository
git clone https://github.com/yourvendor/ndarray
cd ndarray
